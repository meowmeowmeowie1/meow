...
internal static class RPRPvP
{
    ...
    internal class RPRPvP_Burst : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            {
                //Smite
                if (IsEnabled(Preset.RPRPvP_Smite) && PvPMelee.CanSmite() && InActionRange(PvPMelee.Smite) && HasTarget() &&
                    GetTargetHPPercent() <= RPRPvP_SmiteThreshold)
                    return PvPMelee.Smite;

                // Harvest Moon Ranged Option
                if (IsEnabled(Preset.RPRPvP_Burst_RangedHarvest) && GetTargetDistance() > 5 && InActionRange(HarvestMoon))
                    return HarvestMoon;
                ...
                if (IsEnabled(Preset.RPRPvP_Burst_Enshrouded) && enshrouded)
                {
                    if (canWeave)
                    {
                        ...
                        // Lemure's Slice
                        if (ActionReady(LemuresSlice) && canBind && InActionRange(LemuresSlice))
                            return LemuresSlice;
                    }

                    // Communio Option
                    if (IsEnabled(Preset.RPRPvP_Burst_Enshrouded_Communio) &&
                        enshroudStacks == 1 && InActionRange(Communio))
                    {
                ...
                if (!enshrouded)
                {
                    ...
                    {
                        if (IsEnabled(Preset.RPRPvP_Burst_DeathWarrant) && deathWarrantReady && InActionRange(DeathWarrant) &&
                    ...
                    {                               
                        ...
                        if (!IsEnabled(Preset.RPRPvP_Burst_ImmortalPooling) && IsEnabled(Preset.RPRPvP_Burst_DeathWarrant) && deathWarrantReady && InActionRange(DeathWarrant))
                            return OriginalHook(DeathWarrant);

                        // Grim Swathe Option
                        if (IsEnabled(Preset.RPRPvP_Burst_GrimSwathe) && ActionReady(GrimSwathe) && InActionRange(GrimSwathe))
                            return GrimSwathe;
                    }
